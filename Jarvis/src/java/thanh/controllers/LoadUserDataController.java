...
public class LoadUserDataController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (key.equals("LoadListMission")) {
                String txtUsername = request.getParameter("txtUsername");
                MissionBean beans = new MissionBean();
                beans.setUsername(txtUsername);
                beans.setStatus(false);
                List<MissionDTO> listMission = beans.getListMissionOfAUser();
                request.setAttribute("LIST_MISSION", listMission);
                url = USER;
            } else if (key.equals("LoadFinishedMission")) {
                String txtUsername = request.getParameter("txtUsername");
                MissionBean beans = new MissionBean();
                beans.setUsername(txtUsername);
                beans.setStatus(true);
                List<MissionDTO> listMission = beans.getListMissionOfAUser();
                request.setAttribute("LIST_FINISHED_MISSION", listMission);
                url = USER;
            } else {
                request.setAttribute("ERROR", "Action did not supported!");
            }
        } catch (Exception e) {
            log("ERROR at LoadUserDataController" + e.getMessage());
        } finally {
