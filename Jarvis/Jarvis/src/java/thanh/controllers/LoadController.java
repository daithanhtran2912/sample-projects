...
public class LoadController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (key.equals("LoadAllMember")) {
                RegistrationBean beans = new RegistrationBean();
                List<RegistrationDTO> result = beans.getAllMemberToTable();
                HttpSession session = request.getSession();
                String adminName = (String) session.getAttribute("USER");
                for (int i = 0; i < result.size(); i++) {
                    if (result.get(i).getUsername().equals(adminName)) {
                        result.remove(i);
                    }
                }
                request.setAttribute("ALLMEMBER", result);
                url = SUCCESS;
            } else if (key.equals("ViewMemberDetail")) {
                RegistrationBean beans = new RegistrationBean();
                ...
                url = SUCCESS;
            } else if (key.equals("AddNewMember")) {
                url = SUCCESS;
            } else {
